...
function generar_ecuacion(){ 
  var num1 = Math.floor(Math.random() * 10) + 1,
      ...
      dummyrespuesta1 = Math.floor(Math.random() * 10) / 10,
      dummyrespuesta2 = Math.floor(Math.random() * 10),
      todasRespuestas = [],
      switchrespuestas = [];
  ...
  if(num1 > num2){
    ...
    respuesta = eval(num2 / num1);
    document.getElementById("num1").innerHTML = num2; 
    document.getElementById("num2").innerHTML = num1;
  } 

  if(Number.isInteger(respuesta) == false){
    respuesta = respuesta.toFixed(1);
  }
